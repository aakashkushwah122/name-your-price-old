<?php
/**
 * WooCommerce Name Your Price: Db update note.
 *
 * Adds a note to complete the WooCommerce Name Your Price db update after the upgrade in the WC Admin context.
 *
 * @since 3.5.0
 *
...
/**
 * WC_NYP_Notes_Run_Db_Update.
 */
class WC_NYP_Notes_Run_Db_Update {
	const NOTE_NAME = 'wc-nyp-update-db-reminder';
	...
	public function __construct() {
		...
		add_action( 'wc_nyp_hide_update_notice', array( __CLASS__, 'set_notice_actioned' ) );
	...
	private static function update_needed_notice( $note_id = null ) {
		...
		$note_actions = array(
			array(
				'name'         => 'update-db_run',
				'label'        => __( 'Update Database', 'wc_name_your_price' ),
				'url'          => $update_url,
				'status'       => 'unactioned',
				'primary'      => true,
				'nonce_action' => 'wc_nyp_do_update',
				'nonce_name'   => 'wc_nyp_do_update_nonce',
			),
			array(
				'name'    => 'update-db_learn-more',
				'label'   => __( 'Learn more about updates', 'wc_name_your_price' ),
		...
		$note->set_title( __( 'WooCommerce Name Your Price database update required', 'wc_name_your_price' ) );
		$note->set_content(
			__( 'WooCommerce Name Your Price has been updated! To keep things running smoothly, we have to update your database to the newest version.', 'wc_name_your_price' )
			. sprintf( ' ' . esc_html__( 'The database update process runs in the background and may take a little while, so please be patient.', 'wc_name_your_price' ) )
		);
	...
	private static function update_in_progress_notice( $note_id ) {
		...
		$pending_actions_url = admin_url( 'admin.php?page=wc-status&tab=action-scheduler&s=wc_nyp_run_update&status=pending' );
		$cron_disabled       = Constants::is_true( 'DISABLE_WP_CRON' );
		$cron_cta            = $cron_disabled ? __( 'You can manually run queued updates here.', 'wc_name_your_price' ) : __( 'View progress →', 'wc_name_your_price' );

		$note = new Note( $note_id );
		$note->set_title( __( 'WooCommerce Name Your Price database update in progress', 'wc_name_your_price' ) );
		$note->set_content( __( 'The database is updating in the background. The database update process may take a little while, so please be patient.', 'wc_name_your_price' ) );
	...
	private static function update_done_notice( $note_id ) {
		$hide_notices_url = html_entity_decode( // to convert &amp;s to normal &, otherwise produces invalid link.
			add_query_arg(
				array(
					'wc-nyp-hide-notice' => 'update',
				),
				wc_get_current_admin_url() ? remove_query_arg( 'wc_nyp_do_update', wc_get_current_admin_url() ) : admin_url( 'admin.php?page=wc-settings' )
			)
		);
		...
		$note_actions = array(
			array(
				'name'         => 'update-db_done',
				'label'        => __( 'Thanks!', 'wc_name_your_price' ),
				'url'          => $hide_notices_url,
				'status'       => 'actioned',
				'primary'      => true,
				'nonce_action' => 'wc_nyp_hide_notices',
				'nonce_name'   => '_wc_nyp_notice_nonce',
			),
		);
		...
		$note->set_title( __( 'WooCommerce Name Your Price database update done', 'wc_name_your_price' ) );
		$note->set_content( __( 'Database update complete. Thank you for updating to the latest version!', 'wc_name_your_price' ) );
	...
	public static function show_reminder() {
		$needs_db_update = \WC_NYP_Install::needs_db_update();
		...
		if ( ! $needs_db_update ) {
			...
			$next_scheduled_date = WC()->queue()->get_next( 'wc_nyp_run_update_callback', null, 'wc_nyp_db_updates' );

			if ( $next_scheduled_date || ! empty( $_GET['wc_nyp_do_update'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
